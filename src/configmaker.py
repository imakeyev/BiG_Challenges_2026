...
class ConfigGenerator:
    ...
    def _generate_for_simulation(self):
        self.cam_params = {
            ...
            "intrinsics": {
                "fx": self.camera_args['fx'], 
                "fy": self.camera_args['fy'], 
                "cx": self.camera_args['cx'], 
                "cy": self.camera_args['cy']
            },
    ...
    def _generate_for_real(self):
        ...
        if not os.path.exists(self.CAMERA_CALIB_FILE):
            print(f"[ERROR] Calibration module did not create '{self.CAMERA_CALIB_FILE}'. Exiting.")
            sys.exit(1)
...
if __name__ == "__main__":
    ...
    # Аргумент для режима работы
    parser.add_argument(
        '-m', '--mode',
        type=str,
        default='sim',
        choices=['sim', 'real'],
        help="Mode: 'sim' for simulation, 'real' for real hardware"
    )
    ...
    generator = ConfigGenerator(
        simulation_mode=(args.mode == 'sim'),
        camera_args=camera_args_dict
    )
    generator.run()
