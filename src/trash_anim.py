...
OUTPUT_DATA_PATH = os.path.join("C:", "..", "data", "set_10") # Новая папка
# --- ИЗМЕНЕНИЯ ---
TOTAL_FRAMES_TO_GENERATE = 30 # Больше кадров
MIN_ROTATION_SPEED, MAX_ROTATION_SPEED = 0.2, 0.3# Медленнее вращение
LINEAR_SPEED_SCALAR = 0.2# Медленнее полет
# -----------------

# --- Остальные настройки (без изменений) ---
RESOLUTION_X, RESOLUTION_Y = 1280, 720; OBJECT_COLOR_RGB = (0.5, 0.5, 0.5); OBJECT_METALLIC = 0.8; STEREO_BASELINE = 0.7
...
# ... (все функции до __main__ без изменений, я их сверну)
def clear_scene():
...
def import_and_setup_object(filepath):
    ...
    obj = bpy.context.selected_objects[0]; obj.name = "SpaceDebris"; max_dim = max(obj.dimensions); scale_factor = 5.0 / max_dim if max_dim > 0 else 1.0
    obj.scale = (scale_factor, scale_factor, scale_factor); bpy.ops.object.transform_apply(scale=True)
    bpy.ops.object.origin_set(type='ORIGIN_GEOMETRY', center='BOUNDS'); obj.location = (0, 0, -15); obj.rotation_mode = 'XYZ'; return obj
def create_realistic_cv_material(obj):
    mat = bpy.data.materials.new(name="Realistic_CV_Material"); mat.use_nodes = True; nodes = mat.node_tree.nodes; nodes.clear()
    output = nodes.new(type='ShaderNodeOutputMaterial'); principled = nodes.new(type='ShaderNodeBsdfPrincipled'); tex_coord = nodes.new(type='ShaderNodeTexCoord'); noise = nodes.new(type='ShaderNodeTexNoise')
    principled.inputs['Base Color'].default_value = (0.5, 0.5, 0.5, 1.0); principled.inputs['Metallic'].default_value = 0.9; noise.inputs['Scale'].default_value = 150.0; noise.inputs['Detail'].default_value = 16.0; noise.inputs['Roughness'].default_value = 0.7
    links = mat.node_tree.links; links.new(tex_coord.outputs['Generated'], noise.inputs['Vector']); links.new(noise.outputs['Fac'], principled.inputs['Roughness']); links.new(principled.outputs['BSDF'], output.inputs['Surface'])
...
if __name__ == "__main__":
    # ... (код до цикла for, без изменений)
    ...
    if space_debris:
        ...
        print("\n--- НАЧАЛО ГЕНЕРАЦИИ (МЕДЛЕННОЕ ВРАЩЕНИЕ)... ---")
        for frame_num in range(1, TOTAL_FRAMES_TO_GENERATE + 1):
            ...
            cam_pose_matrix = cam_left.matrix_world; pose_filepath = os.path.join(frame_folder, "cam_pose.txt"); np.savetxt(pose_filepath, cam_pose_matrix)
